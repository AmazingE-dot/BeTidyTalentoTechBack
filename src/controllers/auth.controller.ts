...
import UbicacionModel from "../models/ubicacionIp.models";

const enviroment = config[process.env.NODE_ENV || "desarrollo"];

export const login = async (req: Request, res: Response) => {
  const { email, password } = req.body;
  const ipdAdress = enviroment.ip || req.ip;
  ...
  try {
    ...
    if (!usuario) {
      return res.status(401).json({
        ok: false,
        msg: "Las credenciales no son válidas por nombre",
      });
    }

    //Verificar el password
    const validarPassword = bcrypt.compareSync(password, usuario.password);
    console.log(`Password: ${password} Usuario.password: ${usuario.password}`);

    if (!validarPassword) {
      return res.status(401).json({
        ok: false,
        msg: "Las credenciales no son válidas por contraseña",
      });
    }
    ...
    const ubicacionIP = await obtenerUbicacionPorIp(ipdAdress);
...
export const olvidoContrasena = async (req: Request, resp: Response) => {
  ...
  try {
    ...
    if (id) {
      ...
      const name = existeUsuario.nombre;
    ...
    resp.status(400).json({
      ok: false,
      msg: "No se lograrón validar los datos",
    });
  }
};
...
export const cambiarContrasena = async (req: CustomRequest, res: Response) => {
  ...
  try {
    ...
    return res.status(400).json({
      ok: false,
      msg: `Error al actualiar contraseña ${error}`,
    });
  }
};
